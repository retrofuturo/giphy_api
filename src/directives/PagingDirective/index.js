...
/**
 * @ngDoc directive
 * @name ng.directive:PagingDirective
 *
 * @description
 * A directive to aid in PagingDirective large datasets
...
export default function () {
  ...
  function internalAction(scope, page) {
      ...
      // Pass our parameters to the PagingDirective action
      scope.pagingAction()({
          page: scope.page,
   ...
   *
   * @param {int} start - The start of the range to add to the PagingDirective list
   * @param {int} finish - The end of the range to add to the PagingDirective list
   * @param {Object} scope - The local directive scope object
   */
  ...
  /**
   * Add the first or beginning items in our PagingDirective list
   * We leverage the 'next' parameter to determine if the dots are required
   *
   * @param {Object} scope - The local directive scope object
   * @param {int} next - the next page number in the PagingDirective sequence
   */
  ...
  /**
   * Add the last or end items in our PagingDirective list
   * We leverage the 'prev' parameter to determine if the dots are required
   *
   * @param {int} pageCount - The last page number or total page count
   * @param {Object} scope - The local directive scope object
   * @param {int} prev - the previous page number in the PagingDirective sequence
   */
  // Add Last Pages
  ...
  /**
   * The main build function used to determine the PagingDirective logic
   * Feel free to tweak / fork values for your application
   *
   * @param {Object} scope - The local directive scope object
   * @param {Object} attrs - The local directive attribute object
  ...
  function build(scope, attrs) {
      ...
      var fullAdjacentSize = scope.adjacent;

      // Add the Next and Previous buttons to our list
      addPrevNext(scope, pageCount, 'prev');

      // If the page count is less than the full adjacnet size
      // Then we simply display all the pages, Otherwise we calculate the proper PagingDirective display
      if (pageCount <= (fullAdjacentSize + 2)) {
          start = 1;
          addRange(start, pageCount, scope);
      } else {
          // Determine if we are showing the beginning of the PagingDirective list
          // We know it is the beginning if the page - adjacent is <= 2
          if (scope.page <= scope.adjacent) {
              start = 1;
          ...
          // Determine if we are showing the middle of the PagingDirective list
          ...
          else if (scope.page > 2 && scope.page < (pageCount - scope.adjacent)) { 
              start = scope.page - (scope.page - scope.adjacent >= scope.adjacent*2 ? scope.adjacent : (scope.page - scope.adjacent - 1)); // scope.page - scope.adjacent;
          ...
          // If nothing else we conclude we are at the end of the PagingDirective list
          // We know this since we have already ruled out the beginning and middle above
          else {
              const adjacentRange = scope.page - scope.adjacent;
              start = adjacentRange > 2 ? adjacentRange :  pageCount - scope.adjacent;
              finish = pageCount;
      ...
      // Add the next and last buttons to our PagingDirective list
